...
abstract class WebSocketServer {
	...
	abstract protected function process($users,$message,$user); // Called immediately when the data is recieved.
	abstract protected function connected($user); // Called after the handshake response is sent to the client.
	abstract protected function closed($user); // Called after the connection is closed.
	
	protected function connecting($user) {
                
		   $this->stdout("[WebSocketServer] connecting ".$user->id);
	...
	protected function send($user,$message) {
		...
		$this->stdout(">send : $message to ".$user->id);
					...
					elseif ($numBytes == 0) {
						...
						if (!$user->handshake) {
							...
							if (($message = $this->deframe($buffer, $user)) !== FALSE) {
							    $this->stdout("Come in message : ".$message." from user ".$user->id);
								if($user->hasSentClose) {
								   	$this->disconnect($user->socket);
									$this->stdout("Client disconnected. Sent close: " . $user->id);
								} else {
								    $this->process($this->users, $message, $user); // todo: Re-check this. Should already be UTF-8.
								}
							} else {
								do {
									...
									if ($numByte > 0) {
										...
										if (($message = $this->deframe($buffer, $user)) !== FALSE) {
											if($user->hasSentClose) {
											    ...
											    $this->process($this->users,$message, $user);
											}
										}
									}
								} while($numByte > 0);
...
protected function getUserBySocket($socket) {
	//$this->stdout("getUserBySocket...".$socket);
...
protected function extractHeaders($message) {
	...
	'length' => 0,
	'mask' => "");
	$this->stdout("opcode : ".(ord($message[0]) & 15));
